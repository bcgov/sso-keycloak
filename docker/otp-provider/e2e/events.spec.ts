...
test('Send Code Event', async ({ page }, testInfo) => {
  ...
  let requestCodeEvents = await eventModel.findAll({ where: { eventType: 'REQUEST_OTP', email } });
  let resendCodeEvents = await eventModel.findAll({ where: { eventType: 'RESEND_OTP', email } });
  ...
  for (let i = 0; i < Number(config.OTP_RESENDS_ALLOWED_PER_DAY) - 1; i++) {
    ...
    await page.waitForURL('**/otp');
    resendCodeEvents = await eventModel.findAll({ where: { eventType: 'RESEND_OTP', email } });
  ...
  let maxResendEvents = await eventModel.findAll({ where: { eventType: 'MAX_RESENDS', email } });
  ...
  await expect(page.locator('#signin-form')).toMatchAriaSnapshot(`
    ...
    maxResendEvents = await eventModel.findAll({ where: { eventType: 'MAX_RESENDS', email } });
...
test('Retry code event', async ({page}, testInfo) => {
  ...
  let invalidOtpEvents = await eventModel.findAll({ where: { eventType: 'INVALID_OTP', email } });
  let maxAttemptsEvents = await eventModel.findAll({ where: { eventType: 'MAX_ATTEMPTS', email } });
  ...
  for (let i = 0; i <= Number(config.OTP_ATTEMPTS_ALLOWED); i++) {
    ...
    invalidOtpEvents = await eventModel.findAll({ where: { eventType: 'INVALID_OTP', email } });
    expect(invalidOtpEvents.length).toBe(i + 1);

    if (i === Number(config.OTP_ATTEMPTS_ALLOWED)) {
      maxAttemptsEvents = await eventModel.findAll({ where: { eventType: 'MAX_ATTEMPTS', email } });
