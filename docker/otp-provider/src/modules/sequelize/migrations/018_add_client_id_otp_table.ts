...
export const up = async (queryInterface: QueryInterface) => {
  ...
  await queryInterface.sequelize.query(`
    ...
    CREATE OR REPLACE FUNCTION get_otp_wait_time(email_input text, delays json, client_id text, delay_multiplier INT DEFAULT 60)
        RETURNS TABLE(
            can_request BOOLEAN,
            wait_seconds int,
            otp_count int
  ...
  await queryInterface.sequelize.query(`
    ...
    BEGIN
      ...
      INTO can_request, wait_seconds, otp_count from get_otp_wait_time(email_input, delay_minutes_json::JSON, client_id, delay_multiplier) as w;
      ...
      IF otp_count = 0 THEN
        ...
        INSERT INTO "Otp" (id, email, otp, "clientId")
        VALUES (gen_random_uuid(), email_input, otp_input, client_id);
      ...
      IF can_request THEN
        ...
        UPDATE "Otp" set active = false where email = email_input AND "clientId" = client_id;
  ...
  await queryInterface.sequelize.query(`
    ...
    BEGIN
      ...
      FROM "Otp"
      WHERE email = email_input AND active = true AND "clientId" = client_id
      LIMIT 1;
      ...
      DELETE FROM "Otp" WHERE email = email_input AND "clientId" = client_id;
...
export const down = async (queryInterface: QueryInterface) => {
  ...
  await queryInterface.sequelize.query(`DROP FUNCTION IF EXISTS get_otp_wait_time(text, json, text, int);`);
