...
public class OverrideSAMLIdentityProviderFactory extends AbstractIdentityProviderFactory<OverrideSAMLIdentityProvider> {

  public static final String PROVIDER_ID = "custom-saml";
      ...
      if (idpDescriptor != null) {
        ...
        for (EndpointType endpoint : idpDescriptor.getSingleLogoutService()) {
          if (postBindingResponse
            ...
            break;
          } else if (!postBindingResponse
              && endpoint.getBinding().toString().equals(JBossSAMLURIConstants.SAML_HTTP_REDIRECT_BINDING.get())) {
            singleLogoutServiceUrl = endpoint.getLocation().toString();
            break;
          }
