...
public class IdpCreateUserIfUniqueAuthenticator extends AbstractIdpAuthenticator {
  ...
  protected void authenticateImpl(
    ...
    if (duplication == null) {
      ...
      // if (config != null &&
      // Boolean.parseBoolean(config.getConfig().get(IdpCreateUserIfUniqueAuthenticatorFactory.REQUIRE_PASSWORD_UPDATE_AFTER_REGISTRATION))) {
      ...
      context.success();
    } else if (userIdAttrName.equalsIgnoreCase(duplication.getDuplicateAttributeName())
        || usernameAttrName.equalsIgnoreCase(duplication.getDuplicateAttributeName())) {
      UserModel existingUser =
          context.getSession().users().getUserById(duplication.getExistingUserId(), realm);
  ...
  protected ExistingUserInfo checkExistingUser(
    ...
    String userIdAttrName = brokerContext.getIdpConfig().getAlias() + "_user_guid";
    String userIdAttrValue = brokerContext.getUserAttribute(userIdAttrName);
    List<UserModel> existingUserByAttr =
        context
            .getSession()
            .users()
            .searchForUserByUserAttribute(userIdAttrName, userIdAttrValue, context.getRealm());
    if (existingUserByAttr.size() == 1) {
      return new ExistingUserInfo(
          existingUserByAttr.get(0).getId(), userIdAttrName, userIdAttrValue);
    }
    ...
    existingUserByAttr =
        context
            .getSession()
            .users()
            .searchForUserByUserAttribute(usernameAttrName, username, context.getRealm());
    if (existingUserByAttr.size() == 1) {
    ...
    if (brokerContext.getEmail() != null && !context.getRealm().isDuplicateEmailsAllowed()) {
      UserModel existingUser =
          context.getSession().users().getUserByEmail(brokerContext.getEmail(), context.getRealm());
  ...
  // Empty method by default. This exists, so subclass can override and add callback after new user
  // is registered through social
