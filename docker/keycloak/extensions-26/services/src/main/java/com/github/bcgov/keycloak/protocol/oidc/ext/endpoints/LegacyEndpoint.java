...
public class LegacyEndpoint
  ...
  /**
   * This endpoint parses the query params and regenerates them to redirect back
   * to the OIDC logout
   * endpoint. `id_token_hint`, `state`, `ui_locales`, and `initiating_idp` are
   * passed through as
   * they are.
   *
   * <p>
   * 1. if `id_token_hint` exists, it sets the redirect uri to
   * `post_logout_redirect_uri`.<br>
   * 2. if `id_token_hint` omitted, it sets the redirect uri to `redirect_uri`
   * (legacy).
   *
  ...
  public Response logout(
    ...
    UriBuilder uriBuilder = OIDCLoginProtocolService.logoutUrl(session.getContext().getUri(UrlType.FRONTEND));
    ...
    if (redirectUri != null) {
      if (encodedIdToken != null) {
        uriBuilder = uriBuilder.queryParam(OIDCLoginProtocol.POST_LOGOUT_REDIRECT_URI_PARAM, redirectUri);
      } else {
